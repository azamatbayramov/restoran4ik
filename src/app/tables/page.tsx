...
const Page: React.FC = () => {
  ...
  const addTable = () => {
    ...
    const title = newTitle.trim().length === 0 ? `Table ${newTableNumber}` : newTitle.trim();
  ...
  const handleRename = () => {
    ...
    if (editingTableId !== null) {
      setTables((prevTables) =>
        prevTables.map((table) =>
          table.id === editingTableId ? { ...table, title: newTitle.trim() } : table
        )
      );
      setEditingTableId(null);
      setNewTitle('');
        ...
        {editMode && editingTableId !== null && (
          <div className="mb-6 flex items-center">
            <input
              ...
              className={`border ${
                newTitle.length > 14 ? 'border-red-500' : 'border-gray-300'
